...
interface EventsDao {
    ...
    fun getOneTimeEventsFromToWithTypes(toTS: Long, fromTS: Long, eventTypeIds: List<Long>): List<EventEntity>
    ...
    fun getOneTimeEventsFromToWithTypesForSearch(toTS: Long, fromTS: Long, eventTypeIds: List<Long>, searchQuery: String): List<EventEntity>

    @Query("SELECT * FROM events WHERE start_ts <= :toTS AND repeat_interval != 0")
    fun getRepeatableEventsOrTasksWithTypes(toTS: Long): List<EventEntity>

    @Query("SELECT * FROM events WHERE id = :id AND start_ts <= :toTS AND repeat_interval != 0")
    fun getRepeatableEventsOrTasksWithId(id: Long, toTS: Long): List<EventEntity>

    @Query("SELECT * FROM events WHERE start_ts <= :toTS AND start_ts != 0 AND repeat_interval != 0 AND event_type IN (:eventTypeIds)")
    fun getRepeatableEventsOrTasksWithTypes(toTS: Long, eventTypeIds: List<Long>): List<EventEntity>
    ...
    fun getRepeatableEventsOrTasksWithTypesForSearch(toTS: Long, eventTypeIds: List<Long>, searchQuery: String): List<EventEntity>

    @Query("SELECT * FROM events WHERE id IN (:ids) AND import_id != \"\" AND type = $TYPE_EVENT")
    fun getEventsByIdsWithImportIds(ids: List<Long>): List<EventEntity>

    @Query("SELECT * FROM events WHERE source = \'$SOURCE_CONTACT_BIRTHDAY\' AND type = $TYPE_EVENT")
    fun getBirthdays(): List<EventEntity>

    @Query("SELECT * FROM events WHERE source = \'$SOURCE_CONTACT_ANNIVERSARY\' AND type = $TYPE_EVENT")
    fun getAnniversaries(): List<EventEntity>

    @Query("SELECT * FROM events WHERE import_id != \"\"")
    fun getEventsOrTasksWithImportIds(): List<EventEntity>

    @Query("SELECT * FROM events WHERE source = :source AND type = $TYPE_EVENT")
    fun getEventsFromCalDAVCalendar(source: String): List<EventEntity>

    @Query("SELECT * FROM events WHERE id IN (:ids)")
    fun getEventsOrTasksWithIds(ids: List<Long>): List<EventEntity>
    ...
    fun getEventsOrTasksAtReboot(currentTS: Long): List<EventEntity>
    ...
    fun insertOrUpdate(event: EventEntity): Long
